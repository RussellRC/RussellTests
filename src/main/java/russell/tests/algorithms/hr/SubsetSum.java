...
public class SubsetSum {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(hasSubset(9, set, set.length-1));
        System.out.println(hasSubset(10, set, set.length-1));
        System.out.println(hasSubset(13, set, set.length-1));
    ...
    private static boolean hasSubset(int sum, int[] set) {
        ...
        for (int s = 1; s <= sum; s++) {
            for (int i = 1; i <= set.length; i++) {
                ...
                if (s >= set[i-1]) {
                    solution[s][i] = solution[s - set[i-1]][i-1] || solution[s][i];
                }
    ...
    private static boolean hasSubset(int sum, int[] set, int i) {
        ...
        if (set[i] > sum) {
            return hasSubset(sum, set, i-1);
        }
        
        // diff including last element || sum
        return hasSubset(sum-set[i], set, i-1) || hasSubset(sum, set, i-1);
    }
